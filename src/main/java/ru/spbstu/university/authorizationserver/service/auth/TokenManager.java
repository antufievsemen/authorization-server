...
public class TokenManager {
    ...
    public TokenResponseBody generate(@NonNull String clientId, @Nullable String clientSecret, @Nullable String code,
                                      @Nullable String redirectUri, @NonNull List<GrantTypeEnum> grantTypes, @Nullable String accessToken,
                                      @Nullable String refreshToken, @NonNull Optional<String> codeVerifier, @NonNull String sessionId,
        ...
        if (grantTypes.contains(GrantTypeEnum.REFRESH_TOKEN)
            ...
            return refreshAccessTokenResponseBodyGenerator.generate(accessToken, refreshToken);
        }
        if (grantTypes.contains(GrantTypeEnum.AUTHORIZATION_CODE)
                && !grantTypes.contains(GrantTypeEnum.CLIENT_CREDENTIALS)) {
            return authCodeTokenResponseBodyGenerator.generate(client, code, redirectUri, grantTypes, sessionId, codeVerifier);
        }
        if (grantTypes.contains(GrantTypeEnum.CLIENT_CREDENTIALS)
                && !grantTypes.contains(GrantTypeEnum.AUTHORIZATION_CODE)
                && !grantTypes.contains(GrantTypeEnum.REFRESH_TOKEN)) {
            return clientCredentialsTokenResponseBodyGenerator.generate(client, clientSecret, sessionId, scopes);
        }
